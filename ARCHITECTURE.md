...
- **Прод на Railway** — встроенный режим на volume `/data`, одна реплика (`railway.json`). Индекс — 1476 точек, из них 1272 фрагмента PDF ВОЗ; ради такого объёма держать отдельный сервис незачем.
...
**Текущие цифры на слитом коде** (финальный регрессионный прогон 25.09, вывод 21 в [EVALS.md](EVALS.md)). Маршрутизатор, 66 сообщений: верный маршрут 100%, все 18 опасных сообщений ушли тренеру, лишних эскалаций 0%, медиана 1,4 с (`evals/results/router_final_merged.json`). Вопросы, 56 штук: hit@4 86,3%, ответ получили 92,2%, ключевые факты на месте в 86,3%, все вопросы без ответа в корпусе переданы тренеру, медиана 2,8 с, p95 5,8 с, $0,00083 за вопрос, ни одного переключения на резерв (`evals/results/qa_final_merged.json`). Верность источникам 4,70 из 5 (91,5% ответов на 4 и выше) — судья `deepseek-v4.1-flash` оценил все 47 ответов; с прошлыми прогонами, где судил другой судья, напрямую не сравнивается.
...
- **Почему Qdrant и почему встроенный в проде.** Корпус — 1476 фрагментов, сервис — одна реплика, поэтому в проде Qdrant работает внутри процесса на volume Railway: нет второго сервиса, сетевого вызова и его мониторинга. API клиента тот же, что у сервера и Qdrant Cloud, поэтому серверный режим — это переменная `QDRANT_URL`, а не переписывание; он уже поднимается в docker-compose. Переходить на сервер стоит при нескольких репликах, нескольких писателях или заметно большем корпусе (раздел «Qdrant: встроенный режим или сервер»).
...
- Один экземпляр сервиса (SQLite, встроенный Qdrant) — взамен на простоту и изоляцию от базы прода.
