...
Не всё сделано заменяемым — в четырёх местах связь оставлена намеренно:
- **Skill и граф используют одни и те же файлы.** Граф вставляет `SKILL.md` в промпт черновика и запускает тот же `validate_plan.py`, что и Claude. Правила программ живут в одном месте; цена — правка Skill меняет поведение графа, поэтому Skill покрыт тестом на 12 эталонных случаях.
- **Чекпоинты графа и очередь — в SQLite одного процесса.** Проще и изолировано от базы прода Tulpar; цена — один экземпляр сервиса.
- **Бот запущен в том же процессе, что и API.** Один деплой вместо двух; цена — токен бота может опрашивать только один процесс.
...
Документы читает пакет `parsing/`: PyMuPDF — PDF по страницам, python-docx — абзацы и таблицы DOCX в порядке документа (у DOCX нет надёжных страниц, поэтому `page=None`). Затем очистка пробелов с сохранением абзацев и разбиение с перекрытием, выровненным по словам. Пакет не знает про эмбеддинги и Qdrant: он возвращает поля `Chunk`, а индекс остаётся прежним. Переход с pypdf на PyMuPDF и новый сплиттер проверены на 56 вопросах: hit@4 84,3% против 86,3% — разница в один вопрос о PDF ВОЗ из 51, в пределах шума (вывод 14 в [EVALS.md](EVALS.md)).
