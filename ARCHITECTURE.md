...
| Эмбеддинги и реранкер | `rag/embed.py` | Jina → локальный запасной вариант; другой провайдер — один класс |
...
flowchart TD
  ...
  route -- вопрос --> retrieve[retrieve<br/>Qdrant → реранкер]
...
| `corpus/who_2020_physical_activity.pdf` | рекомендации ВОЗ 2020, 104 страницы, английский | постранично, куски ~800 символов с перекрытием 120 | номер страницы сохраняется для цитаты |

Эмбеддинги `jina-embeddings-v3` — многоязычные: вопрос на русском находит английский текст ВОЗ. Поиск: 20 кандидатов из Qdrant → реранкер `jina-reranker-v2-base-multilingual` → 4 лучших. Без ключа Jina работает локальный запасной вариант на хешированных n-граммах с лексическим реранкером — он же базовая линия в A/B.

## Модели и стоимость

| Роль | Основная модель | Резерв | Параметры |
|---|---|---|---|
| route — маршрут сообщения | Ollama Cloud `minimax-m3` | Groq `llama-3.1-8b-instant` | temperature 0, max_tokens 200 |
| text — ответы и черновики | Ollama Cloud `minimax-m3` | Groq `llama-3.3-70b-versatile` | ответ: t 0.2, top_p 0.9, max_tokens 700; черновик: t 0.2, max_tokens 1500 |
| vision — фото еды | Ollama Cloud `kimi-k2.7-code` | Groq `llama-4-scout` | t 0, max_tokens 300 |
| judge — оценка в евалах | Groq `llama-3.3-70b-versatile` | Ollama Cloud `qwen3.5:397b` | t 0 |
| распознавание голоса | Groq `whisper-large-v3-turbo` | — | язык ru |

Выбор опирается на замеры Tulpar: `kimi-k2.7-code` лучше других узнаёт казахстанские блюда (14 из 20 на группе местной кухни) и быстрее; `minimax-m3` почти не выдумывает блюда на фото без еды. Ollama Cloud — подписка, уже работающая в продукте, поэтому нет проблемы с оплатой из Казахстана. Судья в евалах — модель другого семейства, чтобы не оценивать себя самой. Значения гиперпараметров и стоимость одного запроса подтверждаются экспериментами в [EVALS.md](EVALS.md).
